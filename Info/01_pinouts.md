
ILI9341/ST7789V(with Touch)

320x240(Pixel)  2.8-inch  65k Color 

Touch model: MSP2807 
Active area 43.2*57.6 mm 
Power Voltage 3.3V-5V 
Logic - 3.3V
----------------------- 
II                 I .I   pin 14 
II                 I .I
II    LCD screen   I .I
II                 I .I
II                 I .I
II                 I .I   pim 1
-----------------------


Pinouts 

14.  T_IRQ TOUCH_IRQ PB9
13.  T_DO  SPI2_MISO PB14
12.  T_DIN SPI2_MOSI PB15
11.  T_CS  SPI2_CS   PB13      
10.  T_CLK SPI2_SCK  PB10    

9.  MISO   SPI1_MISO  PA6
8.  LED    LED        PC13  (Hi Level - Lighting)
7.  SCK    SPI1_SCK   PA5 
6.  MOSI   SPI1_MOSI  PA7
5.  DC/RC LCD register / data selection signal
	(hi level: register, low level - data) 
4. RESET  RST        PB1  (Low_Level - Reset)
3. CS     SPI1_CS    PB0
2. GND
1. VCC

